...
public class FilmDbStorage implements FilmStorage {
    ...
    public Collection<Film> findAll() {
        return jdbc.query("SELECT id, name, description, release_date, duration, mpa_id FROM films ORDER BY id",
    ...
    public Film update(Film update) {
        ...
        java.time.LocalDate releaseDate = update.getReleaseDate() == null
        ...
        jdbc.update("UPDATE films SET name = ?, description = ?, release_date = ?, duration = ?, mpa_id = ?"
                        + " WHERE id = ?",
    ...
    public List<Film> findPopular(int count) {
        return jdbc.query("SELECT f.id, f.name, f.description, f.release_date, f.duration, f.mpa_id"
    ...
    private Film mapFilm(java.sql.ResultSet rs) throws java.sql.SQLException {
        ...
        film.getLikes().addAll(jdbc.queryForList(
                "SELECT user_id FROM likes WHERE film_id = ? ORDER BY user_id", Long.class, id));
        return film;
    }
    ...
    private Mpa resolveMpa(Mpa mpa) {
        ...
        if (mpa.getId() == null) {
            throw new ru.yandex.practicum.filmorate.exception.ValidationException("У рейтинга МРА должен быть id");
        }
        ...
        if (incoming != null) {
            for (Genre genre : incoming) {
                if (genre == null || genre.getId() == null) {
                    throw new ru.yandex.practicum.filmorate.exception.ValidationException("У жанра должен быть id");
                }
